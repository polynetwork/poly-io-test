...
import (
	...
	"github.com/polynetwork/poly-io-test/chains/fisco/go_abi/fiscox_abi"
	lock_proxy_abi2 "github.com/polynetwork/poly-io-test/chains/fisco/go_abi/lock_proxy_abi"
...
func SendEthCrossFisco(ctx *testframework.TestFrameworkContext, status *testframework.CaseStatus, amount uint64) error {
	gasPrice, err := ctx.EthInvoker.ETHUtil.GetEthClient().SuggestGasPrice(context.Background())
	if err != nil {
		return fmt.Errorf("SendEthCrossFisco, get suggest gas price failed error: %s", err.Error())
	}
	gasPrice = gasPrice.Mul(gasPrice, big.NewInt(5))

	contractabi, err := abi.JSON(strings.NewReader(lock_proxy_abi.LockProxyABI))
	if err != nil {
		return fmt.Errorf("SendEthCrossFisco, abi.JSON error:" + err.Error())
	}

	assetaddress := ethcommon.HexToAddress("0000000000000000000000000000000000000000")
	txData, err := contractabi.Pack("lock", assetaddress, uint64(config.DefConfig.FiscoChainID), ctx.FiscoInvoker.MakeSmartContractAuth().From.Bytes(),
		big.NewInt(int64(amount)))
	if err != nil {
		return fmt.Errorf("SendEthCrossFisco, contractabi.Pack error:" + err.Error())
	}
	...
	gasLimit, err := ctx.EthInvoker.ETHUtil.GetEthClient().EstimateGas(context.Background(), callMsg)
	if err != nil {
		return fmt.Errorf("SendEthCrossFisco, estimate gas limit error: %s", err.Error())
	}
	...
	unsignedTx, err := eth.DeserializeTx(rawtx)
	if err != nil {
		return fmt.Errorf("SendEthCrossFisco, eth.DeserializeTx error: %s", err.Error())
	}
	signedtx, err := types.SignTx(unsignedTx, types.HomesteadSigner{}, ctx.EthInvoker.EthTestSigner.PrivateKey)
	if err != nil {
		return fmt.Errorf("SendEthCrossFisco, types.SignTx error: %s", err.Error())
	}

	err = ctx.EthInvoker.ETHUtil.GetEthClient().SendTransaction(context.Background(), signedtx)
	if err != nil {
		return fmt.Errorf("SendEthCrossFisco, send transaction error:%s", err.Error())
	}
	status.AddTx(signedtx.Hash().String()[2:], &testframework.TxInfo{"EthToFisco", time.Now()})
	//WaitTransactionConfirm(ctx.EthInvoker.ETHUtil.GetEthClient(), signedtx.Hash())
	return nil
}

func SendFiscoEthCrossEthereum(ctx *testframework.TestFrameworkContext, status *testframework.CaseStatus, amount uint64) error {
	...
	txn, err := lp.Lock(ctx.FiscoInvoker.FiscoSdk.GetTransactOpts(), pEthAddr, config.DefConfig.EthChainID,
		ctx.EthInvoker.EthTestSigner.Address.Bytes(), big.NewInt(int64(amount)))
	if err != nil {
		return err
	}
	status.AddTx(txn.Hash().String()[2:], &testframework.TxInfo{"FiscoEthToEthereum", time.Now()})
	isSuccess := ctx.FiscoInvoker.WaitTransactionConfirm(txn.Hash())
	if !isSuccess {
...
func SendFiscoEthCrossFabric(ctx *testframework.TestFrameworkContext, status *testframework.CaseStatus, amount uint64) error {
	...
	txn, err := lp.Lock(ctx.FiscoInvoker.FiscoSdk.GetTransactOpts(), pEthAddr, config.DefConfig.FabricChainID,
		toAcc, big.NewInt(int64(amount)))
	if err != nil {
		return err
	}
	status.AddTx(txn.Hash().String()[2:], &testframework.TxInfo{"FiscoEthToFabric", time.Now()})
	isSuccess := ctx.FiscoInvoker.WaitTransactionConfirm(txn.Hash())
	if !isSuccess {
		return fmt.Errorf("failed to lock")
	}
...
func SendEthCrossFabric(ctx *testframework.TestFrameworkContext, status *testframework.CaseStatus, amount uint64, toAcc []byte) error {
	gasPrice, err := ctx.EthInvoker.ETHUtil.GetEthClient().SuggestGasPrice(context.Background())
	if err != nil {
		return fmt.Errorf("SendEthCrossFabric, get suggest gas price failed error: %s", err.Error())
	}
	gasPrice = gasPrice.Mul(gasPrice, big.NewInt(5))

	contractabi, err := abi.JSON(strings.NewReader(lock_proxy_abi.LockProxyABI))
	if err != nil {
		return fmt.Errorf("SendEthCrossFabric, abi.JSON error:" + err.Error())
	}

	assetaddress := ethcommon.HexToAddress("0000000000000000000000000000000000000000")
	txData, err := contractabi.Pack("lock", assetaddress, uint64(config.DefConfig.FabricChainID), toAcc,
		big.NewInt(int64(amount)))
	if err != nil {
		return fmt.Errorf("SendEthCrossFabric, contractabi.Pack error:" + err.Error())
	}
	...
	gasLimit, err := ctx.EthInvoker.ETHUtil.GetEthClient().EstimateGas(context.Background(), callMsg)
	if err != nil {
		return fmt.Errorf("SendEthCrossFabric, estimate gas limit error: %s", err.Error())
	}
	...
	unsignedTx, err := eth.DeserializeTx(rawtx)
	if err != nil {
		return fmt.Errorf("SendEthCrossFabric, eth.DeserializeTx error: %s", err.Error())
	}
	signedtx, err := types.SignTx(unsignedTx, types.HomesteadSigner{}, ctx.EthInvoker.EthTestSigner.PrivateKey)
	if err != nil {
		return fmt.Errorf("SendEthCrossFabric, types.SignTx error: %s", err.Error())
	}

	err = ctx.EthInvoker.ETHUtil.GetEthClient().SendTransaction(context.Background(), signedtx)
	if err != nil {
		return fmt.Errorf("SendEthCrossFabric, send transaction error:%s", err.Error())
	}
	status.AddTx(signedtx.Hash().String()[2:], &testframework.TxInfo{"EthToFabric", time.Now()})
	//WaitTransactionConfirm(ctx.EthInvoker.ETHUtil.GetEthClient(), signedtx.Hash())
	return nil
}

func SendFabricEthCrossEthereum(ctx *testframework.TestFrameworkContext, status *testframework.CaseStatus, amount uint64) error {
	resp, err := ctx.FabricInvoker.CallChainCode(config.DefConfig.FabricLockProxy, "lock", []string{
		config.DefConfig.FabricPEth,
		strconv.FormatUint(config.DefConfig.EthChainID, 10),
		strings.TrimPrefix(ctx.EthInvoker.EthTestSigner.Address.Hex(), "0x"),
		strconv.FormatUint(amount, 10),
	})
	if err != nil {
		return fmt.Errorf("SendFabricEthCrossEthereum, failed to call chaincode: %v", err)
	}

	status.AddTx(string(resp.TransactionID), &testframework.TxInfo{"EthFromFabricToEthereum", time.Now()})

	return nil
}

func SendFabricEthCrossFisco(ctx *testframework.TestFrameworkContext, status *testframework.CaseStatus, amount uint64) error {
	resp, err := ctx.FabricInvoker.CallChainCode(config.DefConfig.FabricLockProxy, "lock", []string{
		config.DefConfig.FabricPEth,
		strconv.FormatUint(config.DefConfig.FiscoChainID, 10),
		strings.TrimPrefix(ctx.FiscoInvoker.MakeSmartContractAuth().From.Hex(), "0x"),
		strconv.FormatUint(amount, 10),
	})
	if err != nil {
		return fmt.Errorf("SendFabricEthCrossFisco, failed to call chaincode: %v", err)
	}

	status.AddTx(string(resp.TransactionID), &testframework.TxInfo{"EthFromFabricToFisco", time.Now()})
